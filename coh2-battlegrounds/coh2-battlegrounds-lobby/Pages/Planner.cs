...
public sealed class Planner : ViewModelBase {
    ...
    public record LobbyPlanningAction(PlanningObjectiveType ObjectiveType, RelayCommand Click) {
        ...
        public ImageSource? Icon => this.ObjectiveType switch {
            PlanningObjectiveType.OT_Attack => VisualsLookup.ObjectiveTypes[0],
            PlanningObjectiveType.OT_Defend => VisualsLookup.ObjectiveTypes[1],
            PlanningObjectiveType.OT_Support => VisualsLookup.ObjectiveTypes[2],
            _ => null
        };
    ...
    private readonly PlanningOverviewModelInput m_data;
    private readonly ContextHandler m_planningContext;
    ...
    public bool CanStart => this.m_data.Model switch {
        HostLobby => this.m_readyParticipants >= this.m_participantCount,
        ParticipantLobby => this.m_participantCanCancel,
        _ => false
    };

    public string ReturnLobbyText => this.m_data.Model switch { // TODO: Localise
        HostLobby => "Exit Planning",
        _ => "Exit Lobby"
    };

    public string StartLobbyText => this.m_data.Model switch { // TODO: Localise
        HostLobby => this.CanStart ? "Start Match" : $"Waiting for Players ({this.m_readyParticipants}/{this.m_participantCount})",
        ParticipantLobby => this.m_participantIsReady ? (string.IsNullOrEmpty(this.m_participantTitle) ? "Waiting for host" : this.m_participantTitle) : "Ready",
        _ => "Waiting for host"
    };

    public ImageSource? ScenarioPreview => ScenarioPreviewLookup.TryGetMapSource(this.m_data.Scenario);

    public RelayCommand ReturnLobbyCommand { get; }

    public RelayCommand BeginMatchCommand { get; }

    public MutableButton StartMatchButton => this.m_data.Model.StartMatchButton;
    ...
    public Pool<PlanningUnit> PreplacableUnits => this.ContextHandler.PreplacableUnits;

    public LobbyPlanningCompanyDisplay PlanningDisplay { get; } = new("", new(0, 0));

    public ContextHandler ContextHandler => this.m_planningContext;
    ...
    public Planner(PlanningOverviewModelInput input) {
        ...
        Application.Current.Dispatcher.Invoke(this.DisplayWorldElements);
    ...
    private void OnLobbyParticipantMatchInfo(LobbyMatchInfoEventArgs e) {
        if (e.Type is "upload_status") {
            Application.Current.Dispatcher.Invoke(() => {
                this.m_participantTitle = VisualsLookup.LOCSTR_DOWNLOAD(e.Reason);
    ...
    private void OnLobbyParticipantMatchStart() {
        Application.Current.Dispatcher.Invoke(() => {
            this.m_participantTitle = VisualsLookup.LOCSTR_PLAYING();
            this.m_participantCanCancel = false;
            this.Notify(nameof(CanStart));
            this.Notify(nameof(StartLobbyText));
        });
    ...
    private void ExitPlanning() {
        ...
        if (this.m_data.Model is ParticipantLobby pModel) {
            ...
            // Change our view
            GetViewManager().UpdateDisplay(AppDisplayTarget.Right, this.m_data.Model);
    ...
    private static LobbyPlanningParticipantDisplay OccupantToDisplay(ILobbyMember occupant, ValRef<int> i, ValRef<int> j, bool columInverse, int pid) {
        ...
        if (occupant.Company is ILobbyCompany company) {
            return new LobbyPlanningParticipantDisplay(VisualsLookup.FactionIcons[company.Army], name, company.Name, row, c, p);
        } else {
    ...
    private void BeginMatch() {
        ...
        if (this.m_data.Model is HostLobby hostModel) {
        ...
        } else if (this.m_data.Model is ParticipantLobby participantModel) {
