...
namespace Battlegrounds.Game.DataCompany {
    ...
    public class UnitBuilder : IBuilder<Squad> {
        ...
        public virtual UnitBuilder SetTransportBlueprint(string sbpName) 
            => this.SetTransportBlueprint(BlueprintManager.FromBlueprintName(sbpName, BlueprintType.SBP) as SquadBlueprint);
        ...
        public static UnitBuilder EditUnit(Squad squad) {
            ...
            // Grab elements
            var upgrades = squad.Upgrades.ToArray().Map(x => x as UpgradeBlueprint).Filter(x => x is not null);
            var items = squad.SlotItems.ToArray().Map(x => x as SlotItemBlueprint).Filter(x => x is not null);
