...
namespace Battlegrounds.Game.Blueprints.Collections;
...
public sealed class BlueprintCollection<T> : IEnumerable<T> where T : Blueprint {
    ...
    /// </summary>
    public IEnumerable<BlueprintUID> PropertyBagIDs => m_internalDictionary.Select(x => x.Key);
    ...
    public BlueprintCollection(Dictionary<BlueprintUID, Blueprint> dictionaries) {
        m_internalDictionary = dictionaries.Select(x => new KeyValuePair<BlueprintUID, T>(x.Key, (T)x.Value));
    }
    ...
    public T Single(Predicate<T> predicate) => m_internalDictionary.FirstOrDefault(x => predicate(x.Value)).Value;
    ...
    public BlueprintCollection<T> FilterByMod(ModGuid modguid) => new BlueprintCollection<T>(m_internalDictionary.Where(x => x.Value.PBGID.Mod == modguid));
    ...
    /// <param name="modguid">The mod GUID to filter by.</param>
    /// <returns>A new <see cref="BlueprintCollection{T}"/> containing only <typeparamref name="T"/> instances of specifiied mod GUID.</returns>
    public BlueprintCollection<T> FilterByMod(string modguid) => new BlueprintCollection<T>(m_internalDictionary.Where(x => x.Value.PBGID.Mod == modguid));
    ...
    public BlueprintCollection<T> Filter(Predicate<T> predicate) => new BlueprintCollection<T>(m_internalDictionary.Where(x => predicate(x.Value)));

    ///<inheritdoc/>
    public IEnumerator<T> GetEnumerator() => m_internalDictionary.Select(x => x.Value).GetEnumerator();

    ///<inheritdoc/>
    IEnumerator IEnumerable.GetEnumerator() => m_internalDictionary.Select(x => x.Value).GetEnumerator();
