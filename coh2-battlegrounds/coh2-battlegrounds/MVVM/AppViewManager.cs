...
public class AppViewManager {
    ...
    public void SetDisplay(AppDisplayState displayState, ViewModelType left, ViewModelType right) {
        if (displayState == AppDisplayState.LeftRight) {
            this.UpdateDisplayInternal(AppDisplayTarget.Left, this.SolveDisplay(left), null);
            this.UpdateDisplayInternal(AppDisplayTarget.Right, this.SolveDisplay(right), null);
        } else if (displayState == AppDisplayState.Full) {
            this.UpdateDisplayInternal(AppDisplayTarget.Full, this.SolveDisplay(left) ?? this.SolveDisplay(right), null); // Use left, and right if left was invalid.
        } else {
    ...
    public void UpdateDisplay(AppDisplayTarget target, ViewModelType display) {
        if (this.SolveDisplay(display) is IViewModel model) {
            this.UpdateDisplayInternal(target, model, null);
        }
    }

    public void UpdateDisplay<T>(AppDisplayTarget target) where T : IViewModel
        => this.UpdateDisplay<T>(target, null);

    public void UpdateDisplay<T>(AppDisplayTarget target, OnViewSwitched? switchCallback) where T : IViewModel {
        if (this.SolveDisplay(typeof(T)) is T model) {
            this.UpdateDisplayInternal(target, model, switchCallback);
        } else {
            throw new InvalidOperationException();
        }
    }

    private void UpdateDisplayInternal(AppDisplayTarget target, IViewModel? model, OnViewSwitched? switchCallback) {
