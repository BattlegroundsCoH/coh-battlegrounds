...
public sealed class BlueprintSymbolConverter : IValueConverter {

    private static readonly ILogger logger = Log.ForContext<BlueprintSymbolConverter>();
    ...
    public object? Convert(object value, Type targetType, object parameter, CultureInfo culture) {
        if (value is Blueprint blueprint && blueprint.TryGetExtension<UIExtension>(out var uiExtension)) {
            string path = $"Assets/Factions/{parameter}/symbols/{uiExtension.SymbolIconName}.png";
            if (!File.Exists(path)) {
                logger.Warning("Symbol file not found for blueprint {Blueprint}: {Path}", blueprint.Id, path);
...
public sealed class BlueprintSymbolMultiBindingConverter : IMultiValueConverter {
    private readonly BlueprintSymbolConverter _converter = new();
