...
public sealed class BlueprintPortraitConverter : IValueConverter {

    private static readonly ILogger logger = Log.ForContext<BlueprintPortraitConverter>();
    ...
    public object? Convert(object value, Type targetType, object parameter, CultureInfo culture) {
        if (value is Blueprint blueprint && blueprint.TryGetExtension<UIExtension>(out var uiExtension)) {
            string path = $"Assets/Factions/{parameter}/portraits/{uiExtension.PortraitName}.png";
            if (!File.Exists(path)) {
...
public sealed class BlueprintPortraitMultiBindingConverter : IMultiValueConverter {
    private readonly BlueprintPortraitConverter _converter = new();
